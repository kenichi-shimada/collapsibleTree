...
#' @export
collapsibleTreeNetwork.ks <- function(df, inputId = NULL, attribute = "leafCount",
                                    aggFun = sum, fill = "#ccc",#"lightsteelblue",
  ...
  if(!"key" %in% names(df)) stop("information is passed to another plot through 'key' column")
  ...
  # key exists
  if(!is.null(inputId)) jsonFields <- c(jsonFields, "key")
